			...
			if (input == 1) {
				...
				if(input4==1) {
					...
					for(int i = 0; i<inventory.size();i++){
						if(inventory.get(i).getName().equals(input3)) inventory.get(i).emptyCup(amount);
					}
					...
					for(int i = 0; i<inventory.size();i++){
						if(inventory.get(i).getName().equals(input3)) {
							...
							System.out.println(input3+" has been dropped. \nIs broken: "+inventory.get(i).getBroken());
						}
					}
				} else if (input4==3){
					for(int i = 0; i<inventory.size();i++){
						if(inventory.get(i).getName().equals(input3)) inventory.get(i).breakCup();
					}
...
/*
 * ADD BROKEN STATUS TO THE INVENTORY REPORT
 */
	public void buyGlass(String cupName, String cupColor, int cupSerialNumber, boolean cupHandle, double cupMaxFluid,double cupCurrentFluid) {
	...
	public void buyTumbler(String cupName, String cupColor, int cupSerialNumber , double cupMaxFluid,double cupCurrentFluid) {
	...
	public static void sellCup(String input2) {
		for(int i = 0; i<inventory.size();i++){
			if(inventory.get(i).getName().equals(input2))inventory.remove(i);
		}
	}
	...
	public static void getInfo() {
		System.out.println("Name\t\tColor\t\tSerial\t\tMax Fluid\tCurrent Fluid");
		for(int i = 0 ; i < inventory.size(); i++){
			System.out.println(inventory.get(i).getName() + "\t\t" +inventory.get(i).getColor() + "\t\t" + inventory.get(i).getSerialNumber() + "\t\t" + inventory.get(i).getMaxFluid() + "\t\t" + inventory.get(i).getCurrentFluid());
		}
		System.out.println();
	}
}
